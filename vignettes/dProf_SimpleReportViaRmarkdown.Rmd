...
```{r col_summary, echo=FALSE, results="asis", fig.height= 2}
for(ith_tbl in tbl_sum$table_id) {
  ...
  for(ith_col in col_sum$column_id) {
    ...
    print(kable(kt1,
                col.names = c("# Rows", "# NULLs", "# Distinct",
                             "# Empty", "# Numeric", "# Date")))
    kt2 <- col_sum_i %>%
    ...
    if (bars$plot_type[1] == "histogram") {
      ...
      p <- ggplot(bars, aes(x_var, count)) + geom_bar(stat = "identity") +
             xlab("Value") + ylab("# Rows")
    } else {
      XLab <- ifelse(bars$plot_type[1] == "first character", "1st Character of String",
